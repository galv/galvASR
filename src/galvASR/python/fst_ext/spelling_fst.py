...
def create_spelling_fst(word_table, alphabet_table, repeat_char,
  ...
  compiler = openfst.Compiler(fst_type="const", arc_type="log",
                              isymbols=alphabet_table, osymbols=word_table,
  ...
  for word in openfst.SymbolTableIterator(word_table):
    if word not in word_table:
      raise ValueError("Word {0} not in vocabulary {1}".format(word, word_table))
    for i, char in enumerate(word):
      if char not in alphabet_table:
        raise ValueError("Character {0} not in alphabet {1}"
                         .format(char, alphabet_table.name()))
      # Edge case: single-letter word?
      if i == 0:
        build_fst(edge_format_str.format(start=start_state_index,
                                         end=state_index if len(word) != 1 else final_state_index,
                                         char=char, word=WORD_EPSILON_STR,
        ...
        if i == len(word) - 1:
          build_fst(edge_format_str.format(start=state_index,
          ...
          build_fst(edge_format_str.format(start=state_index,
                                           ...
                                           char=char, word=word,
                                           weight=next_transition_cost))
          build_fst(edge_format_str.format(start=state_index, end=state_index,
                                           char=ALPHABET_EPSILON_STR,
                                           word=WORD_EPSILON_STR,
                                           weight=self_transition_cost))
      state_index += 1
