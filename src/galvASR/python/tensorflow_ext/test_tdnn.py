...
def test_tdnn():
  ...
  model = tf.estimator.Estimator(
      tdnn.tdnn_model_fn_with_cross_entropy, params=params)

  batch_size = 16
  time_steps = 20
  input_feature_size = 40
  x = np.random.normal(size=(time_steps, batch_size,
                             input_feature_size)).astype(np.float32)
  y = np.random.randint(
      0, params['num_labels'], size=(time_steps, batch_size)).astype(np.int32)
